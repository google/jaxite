...
def _shape_generator(
    ...
    shape: Sequence[int],
    dtype: jnp.dtype = jnp.uint32,  # pyrefly: ignore[bad-function-definition]
) -> jax.Array:
...
class RandomSource(abc.ABC):
  ...
  @abc.abstractmethod
  def uniform(
      self, shape: Sequence[int] = (1,), dtype: jnp.dtype = jnp.uint32  # pyrefly: ignore[bad-function-definition]
  ) -> jax.Array:
