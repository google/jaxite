...
from jaxite.jaxite_lib.rns import gen_rns_polynomial
from jaxite.jaxite_lib.rns import RnsParams
from jaxite.jaxite_lib.rns import RnsPolynomial
...
def _bit_reversal(x: int, num_bits: int) -> int:
  """Bit-reversal of x with num_bits representation."""
  result = 0
...
def _bit_reversal_array(xs: list[Any]) -> None:
  """Bit-reverse the given array xs in place."""
...
class CkksEncoder:
  ...
  def __post_init__(self):
    if not _is_power_of_two(self.degree):
    ...
    _bit_reversal_array(self.psis_bitrev_inv)
  ...
  def encode(self, values: list[complex]) -> RnsPolynomial:
    ...
    _bit_reversal_array(coeff_values)
  ...
  def _crt(self, coeffs_qs: list[list[int]], qs: list[int]) -> list[int]:
    ...
    q_hat_invs = [_inverse_mod(q_hats[i], qs[i]) for i in range(num_moduli)]
...
class CkksCiphertext:
  ...
  def to_jnp_array(self) -> Tuple[jnp.ndarray, jnp.ndarray]:
    return (
        ...
        jnp.array(
            [c.to_jnp_array() for c in self.components]
        ),
    )
