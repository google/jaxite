...
def _inverse_mod(x: int, q: int) -> int:
...
def _num_bits(x: int) -> int:
  """Returns the number of bits in x."""
  return x.bit_length() - 1


def _bit_reversal(x: int, num_bits: int) -> int:
...
def _bit_reversal_array(xs: list[Any]) -> None:
  """Rearrange the given array in bit-reversal order in place."""
  num_bits = _num_bits(len(xs))
  for i in range(len(xs)):
    j = _bit_reversal(i, num_bits)
    if i < j:
      xs[i], xs[j] = xs[j], xs[i]
...
def _primitive_root(m: int, q: int):
  ...
  for t in range(2, q):
    ...
    # t^k is a primitive root if (t^k)^n != 1 mod q, i.e. it is -1 mod q.
...
class Ntt:
  ...
  def __post_init__(self):
    ...
    n = self.n
    q = self.q
    self.n_inv_mod_q = _inverse_mod(n, q)
    ...
    _bit_reversal_array(self.psis_bitrev)
    ...
    _bit_reversal_array(self.psis_inv_bitrev)
  ...
  def forward(self, coeffs: list[int]) -> None:
    """Forward NTT."""
    self._iterative_cooley_tukey(coeffs, _num_bits(len(coeffs)))

  def backward(self, coeffs: list[int]) -> None:
    """Backward NTT (normalized)."""
    self._iterative_gentleman_sande(coeffs, _num_bits(len(coeffs)))
...
class RnsPolynomial:
  ...
  def _check_other(self, other: 'RnsPolynomial') -> None:
    """Check if two polynomials are compatible."""
    if self.degree != other.degree:
      raise ValueError('`degree` must be the same.')
    if len(self.moduli) != len(other.moduli):
      raise ValueError('`moduli` must have the same length.')
    for i in range(len(self.moduli)):
      if self.moduli[i] != other.moduli[i]:
        raise ValueError('`moduli` must have the same moduli.')
    if len(self.coeffs) != len(other.coeffs):
      raise ValueError('`coeffs` must have the same length.')
    if self.is_ntt != other.is_ntt:
      raise ValueError('`is_ntt` must be the same.')
