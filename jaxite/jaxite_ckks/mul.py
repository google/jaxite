...
class Mul:
  ...
  moduli) rather than handling multiple levels dynamically. Rescaling is
  also not handled automatically by this kernel, and the user (or compiler) is
  expected to insert rescale operations before or after multiplication ops.
  ...
  def precompute_constants(
    ...
    # This check determines if bconv is initialized
  ...
  def tensor_multiply(self, ct1: Ciphertext, ct2: Ciphertext) -> Ciphertext:
    """Performs tensor multiplication of two ciphertexts."""
    if not self.is_initialized:
      raise ValueError("Constants must be precomputed first.")

    a0 = ct1.data[0].astype(jnp.uint64)
    a1 = ct1.data[1].astype(jnp.uint64)
    b0 = ct2.data[0].astype(jnp.uint64)
    b1 = ct2.data[1].astype(jnp.uint64)

    moduli = ct1.moduli.astype(jnp.uint64)
