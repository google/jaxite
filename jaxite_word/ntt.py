...
def tonelli_shanks(q, omega):
  ...
  while t != 1:
    ...
    while temp != 1:
      ...
      i += 1
      if i == m:
        raise Exception("Algorithm failed to find the exponent i")
    # Update values
    b = pow(c, 2 ** (m - i - 1), q)
    m = i
    c = pow(b, 2, q)
...
def intt_four_step(X, q, omega, rows, cols):
  ...
  Parameters:
    ...
    q     : prime modulus.
    rows, cols  : factors of N, so that N = rows * cols.
  ...
  # The forward transform did: X = flatten(transpose(Z)) with Z of shape (rows, cols).
...
@jax.jit
def hpmatmul_bat_adapt_step1_batch(lhs: jax.Array, y: jax.Array):
...
@jax.jit
def hpmatmul_bat_adapt_step3_batch(y: jax.Array, rhs: jax.Array):
...
def ntt_layout_invariant_batch(
  ...
  result_step1 = hpmatmul_bat_adapt_step1_batch(tf_step1, poly_coef_2d)
  ...
  result_step3 = hpmatmul_bat_adapt_step3_batch(result_step2_mod_q, tf_step3)
