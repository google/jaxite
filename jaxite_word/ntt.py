...
def intt_four_step(x, q, omega, rows, cols):
  ...
  Args: x     : list or 1D numpy array (length N) that is the forward NTT
  result. q     : prime modulus. omega : the primitive N-th root of unity used
  in the forward transform. (Forward transform used: rowNTT with omega_row =
  omega^R and columnNTT with omega_col = omega^C, plus twiddle multiplication
  T[r,c] = omega^(r*c).) rows  : factors of N, so that N = rows * cols. cols  :
  factors of N, so that N = rows * cols.
...
def ntt_negacyclic_tpu_algorithm(
  ...
  Args: a   : list (or 1D array) of integers (length n). q   : prime modulus.
  psi : an element in GF(q) such that psi^(2*n) = 1 and psi^n = -1 mod q. (That
  is, psi is a primitive 2n-th root of unity; note that then ω = psi^2 is a
  primitive n-th root of unity.)
    rows: Number of rows in the matrix.
