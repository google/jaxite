...
class CPUDistributionMSMContextBase(MultiscalarMultiplicationContextBase):
  ...
  def __init__(self, parameters: dict):
    ...
    self.scalar_bits: int = 0
    self.slice_bits: int = 0
    self.tile_length: int = 0
    self.msm_length: int = 0
    self.order: int = 0
    self.tile_num: int = 0
    self.window_num: int = 0
    self.bucket_num_per_window: int = 0
    self.bucket_num_last_window: int = 0
    self.moduli_num: int = 0
    self.special_duplication_ratio: int = 0
    self.bucket_num_duplication: int = 0
    self.coordinate_dim: int = 4
    self.zero_point: Any = None
...
class OldCPUDistributionMSMContext(CPUDistributionMSMContextBase):
  ...
  def _init_config_parameters(self):
    ...
    self.msm_length = self.parameters.get("msm_length", 0)
    self.tile_length = self.parameters.get("tile_length", 0)
    assert (
    ...
    self.slice_bits = self.parameters.get("slice_bits", 0)
    self.scalar_bits = self.parameters.get("scalar_bits", 0)
    self.order = self.parameters.get("order", 0)
  ...
  def distribute_buckets(
      self, tiled_slices: jnp.ndarray, tiled_points: jnp.ndarray, *args, **kwargs
...
class TPUDistributionMSMContext(CPUDistributionMSMContext):
  ...
  def distribute_buckets(self, regular_slices, last_window_slice, tiled_points, *args, **kwargs):
    if self.use_compiled_kernels:
      kernel_hash = hash_args(
...
class FusionMSMContext(
    CPUDistributionMSMContextBase, JaxKernelContextBase
):
  ...
  def to_computational_format(
    ...
    if self.use_sharding:
      assert self.bba_reg_slices_padded is not None
  ...
  def _bba_and_bws(
    ...
    if self.use_sharding:
      assert self.sharding_mesh is not None
  ...
  def _place_sharded_state(self):
    ...
    assert (
        self.bba_reg_buckets_padded is not None
        and self.bba_last_buckets_padded is not None
        and self.bws_window_sum_padded is not None
    )
