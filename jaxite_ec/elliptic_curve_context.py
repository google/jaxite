...
class EllipticCurveContextBase(ABC):
  ...
  def __init__(self, parameters: dict):
    ...
    prime = parameters.get("prime", None)
    assert prime is not None, "prime must be provided"
...
class ExtendedTwistedEdwardsContextBase(EllipticCurveContextBase):
  ...
  def _convert_to_weierstrass_affine(self, coordinates: list[Any]) -> list[int]:
    assert (
        len(coordinates) == 4
...
class ExtendedTwistedEdwardsContext(
  ...
  def to_original_format(self, a: jnp.ndarray) -> list:
    ...
    if dim == 2:
      assert isinstance(a_orig, list)
      affine_coords = self._convert_to_weierstrass_affine(a_orig)
    elif dim == 3:
      assert isinstance(a_orig, list)
      affine_coords = [self._convert_to_weierstrass_affine(a_i) for a_i in a_orig]
    else:
      raise ValueError(
